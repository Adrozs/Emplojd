...
const DatePick = ({ name, labelText, disabled = false, onDateChange }) => {
  const [startdate, setStartDate] = useState(new Date());
  const [enddate, setEndDate] = useState(new Date());

  const inputClassName = () => {
    let baseClass =
      "w-full rounded-xl border-2 outline-sky-800 px-4 py-2 dark:bg-slate-700";
    if (disabled) {
      return (
        baseClass +
        " border-gray-300 dark:border-gray-600 bg-transparent dark:bg-transparent cursor-not-allowed"
      );
    } else {
